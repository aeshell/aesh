...
public class Parser {
    ...
    public static String formatDisplayListTerminalString(List<TerminalString> displayList, int termHeight, int termWidth) {
        ...
        for(int i=0; i < numRows; i++) {
            for(int c=0; c < numColumns; c++) {
                int fetch = i + (c * numRows);
                if(fetch < displayList.size())
                    completionOutput.append(padRight(maxLength, displayList.get(i + (c * numRows)).getCharacters().toString())) ;
                else
                    break;
            }
            completionOutput.append(Config.getLineSeparator());
    ...
    public static String findStartsWithOperation(List<CompleteOperation> coList) {
        ...
        for(CompleteOperation co : coList) {
            String s = findStartsWithString(co.getFormattedCompletionCandidatesAsString());
            if(s.length() > 0)
                tmpList.add(s);
            else
                return "";
        }
        return findStartsWithString(tmpList);
    }
     ...
     */
    public static String findStartsWithString(List<String> completionList) {
        StringBuilder builder = new StringBuilder();
        for(String completion : completionList)
            while(builder.length() < completion.length() &&
                  startsWithString(completion.substring(0, builder.length()+1), completionList))
     ...
     * @return biggest common startsWith string
     */
    public static String findStartsWith(List<TerminalString> completionList) {
        StringBuilder builder = new StringBuilder();
        for(TerminalString completion : completionList)
            while(builder.length() < completion.getCharacters().length() &&
                  startsWith(completion.getCharacters().substring(0, builder.length()+1), completionList))
                builder.append(completion.getCharacters().charAt(builder.length()));

        return builder.toString();
    }

    private static boolean startsWithString(String criteria, List<String> completionList) {
        for(String completion : completionList)
    ...
    private static boolean startsWith(String criteria, List<TerminalString> completionList) {
        for(TerminalString completion : completionList)
