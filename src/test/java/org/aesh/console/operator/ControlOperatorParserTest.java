...
public class ControlOperatorParserTest {
    ...
    public void testRedirectionOperation() {
        Assert.assertEquals(new ConsoleOperation(ControlOperator.NONE, "ls foo.txt"),
                ControlOperatorParser.findAllControlOperators("ls foo.txt").get(0));

        Assert.assertEquals(new ConsoleOperation(ControlOperator.OVERWRITE_OUT, "ls . "),
                ControlOperatorParser.findAllControlOperators("ls . > foo.txt").get(0));
        Assert.assertEquals(new ConsoleOperation(ControlOperator.NONE, " foo.txt"),
                ControlOperatorParser.findAllControlOperators("ls . > foo.txt").get(1));

        Assert.assertEquals(new ConsoleOperation(ControlOperator.OVERWRITE_OUT, "bas "),
                ControlOperatorParser.findAllControlOperators("bas > foo.txt 2>&1 ").get(0));
        Assert.assertEquals(new ConsoleOperation(ControlOperator.OVERWRITE_OUT_AND_ERR, " foo.txt "),
        ...
        Assert.assertEquals(new ConsoleOperation(ControlOperator.OVERWRITE_OUT, "ls \\<foo.txt\\> "),
                ControlOperatorParser.findAllControlOperators("ls \\<foo.txt\\> > test.txt").get(0));

        Assert.assertEquals(new ConsoleOperation(ControlOperator.NONE, "ls bar=>foo"),
