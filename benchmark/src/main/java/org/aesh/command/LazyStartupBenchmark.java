...
public class LazyStartupBenchmark {
    ...
    @CommandDefinition(name = "bench", description = "bench root", groupCommands = {
    ...
    private static long runOnce(boolean lazy) {
        ...
        CommandResult result = lazy
                ? AeshRuntimeRunner.builder().lazyStartup(true).command(BenchRoot.class).args("c00").execute()
                : AeshRuntimeRunner.builder().command(BenchRoot.class).args("c00").execute();
        long elapsed = System.nanoTime() - start;
        if (result != CommandResult.SUCCESS)
            throw new IllegalStateException("Unexpected result: " + result);
        int constructed = BaseChild.constructions.get() + BenchRoot.constructions.get();
        int expected = lazy ? 2 : CHILD_COUNT + 1;
        if (constructed != expected)
            throw new IllegalStateException(
                    "Expected " + expected + " constructions, got " + constructed + " (lazy=" + lazy + ")");
        return elapsed;
    }
    ...
    public static void main(String[] args) {
        ...
        long eagerMean = eagerTotal / MEASURED_ITERATIONS;
        long lazyMean = lazyTotal / MEASURED_ITERATIONS;
        ...
        System.out.println("Eager  | " + String.format("%13d", CHILD_COUNT + 1) + " | "
                + TimeUnit.NANOSECONDS.toMicros(eagerMean) + " us");
        System.out.println("Lazy   | " + String.format("%13d", 2) + " | "
                + TimeUnit.NANOSECONDS.toMicros(lazyMean) + " us");
    }
}
