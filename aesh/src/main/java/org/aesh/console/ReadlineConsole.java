...
public class ReadlineConsole implements Console, Consumer<Connection> {
    ...
    private void processLine(String line, Connection conn) {
        ...
        if (settings.enableShellEscape() && line.trim().startsWith("!")) {
            ...
            if (!nativeCmd.isEmpty()) {
                processManager.runNative(new NativeExecution(conn, nativeCmd), conn, line);
            }
            read(conn, readline);
            return;
        }
        ...
        try {
            ...
            if (settings.commandNotFoundHandler() != null) {
                settings.commandNotFoundHandler().handleCommandNotFound(line,
                        ...
                        cnfe.getCommandName(),
                        settings.commandRegistry().getAllCommandNames());
            } else {
    ...
    private CommandRuntime<? extends CommandInvocation> generateRuntime() {
        shell = new ShellImpl(connection, settings.enableSearchInPaging(), settings);
