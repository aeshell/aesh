...
class FileCompleterGenerator {
    ...
    private String generateMainCompletion(CommandLineParser<CommandInvocation> command) {
        ...
        main.append("function _complete_").append(command.getProcessedCommand().name().toLowerCase()).append(" {")
                .append(getLineSeparator());
        if (command.isGroupCommand()) {
            for (int i = 0; i < command.getAllChildParsers().size(); i++) {
                main.append("  CHILD").append(i).append("=(")
                        .append(command.getAllChildParsers().get(i).getProcessedCommand().name().toLowerCase()).append(")")
                        .append(getLineSeparator());
            }
            for (int i = 0; i < command.getAllChildParsers().size(); i++) {
                main.append("  ArrContains COMP_WORDS CHILD").append(i)
                        .append(" && { _command_")
                        .append(command.getAllChildParsers().get(i).getProcessedCommand().name().toLowerCase())
                        .append("; return $?; }").append(getLineSeparator());
            }

            main.append(Config.getLineSeparator());
        }
        main.append("  _command_").append(command.getProcessedCommand().name().toLowerCase()).append("; return $?;")
                .append(getLineSeparator());
    ...
    private String generateCommand(CommandLineParser<CommandInvocation> command) {
        ...
        builder.append("function _command_").append(command.getProcessedCommand().name().toLowerCase()).append(" {")
                .append(getLineSeparator());
        ...
        builder.append(
                "  COMPREPLY=( $(compgen -W \"${NO_VALUE_OPTIONS} ${VALUE_OPTIONS} ${CHILD_COMMANDS}\" -- ${CURR_WORD}) )")
                .append(getLineSeparator());
        ...
        if (builder.length() > 0) {
            ...
            for (ProcessedOption option : command.getProcessedCommand().getOptions()) {
                if (option.hasValue()) {
                    ...
                    builder.append(")").append(getLineSeparator());
                    builder.append("      COMPREPLY=( $( compgen -W \"${").append(option.name().toUpperCase())
                            .append("_VALUES}\" -- \"\" ) )").append(getLineSeparator());
            ...
            for (ProcessedOption option : command.getProcessedCommand().getOptions()) {
                if (option.hasValue()) {
                    ...
                    builder.append(")").append(getLineSeparator());
                    builder.append("      COMPREPLY=( $( compgen -W \"${").append(option.name().toUpperCase())
                            .append("_VALUES}\" -- $CURR_WORD ) )").append(getLineSeparator());
                    builder.append("      return $?").append(getLineSeparator()).append("      ;;").append(getLineSeparator());
                }
            }
            builder.append("    esac").append(getLineSeparator()).append("  esac").append(getLineSeparator())
                    .append(getLineSeparator());
