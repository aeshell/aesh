...
image:https://img.shields.io/github/license/aeshell/aesh[License,link=http://www.apache.org/licenses/LICENSE-2.0]
...
* Terminal charts library (`aesh-charts`): line charts, bar charts, sparklines
...
* Structured (pre-tokenized) command execution -- no quoting needed for special characters
* Command output capture separate from readline chrome (for test frameworks)
* `onReady` callback for reliable REPL startup synchronization
* POSIX-aligned `CommandResult` exit codes with consistent propagation
...
Aesh is licensed under the http://www.apache.org/licenses/LICENSE-2.0[Apache License, Version 2.0].
